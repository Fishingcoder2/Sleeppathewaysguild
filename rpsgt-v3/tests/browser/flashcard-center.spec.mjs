...
test('complete RPSGT deck renders as interactive sticky notes and preserves focused study controls',async({page})=>{
  ...
  await expect(page.locator('[data-card-grid] .deck-note')).toHaveCount(312);
  await expect(page.locator('[data-card-summary]')).toContainText('312 shown');
  await expect(page.locator('[data-card-summary]')).toContainText('312 total');
  ...
  await expect(page.locator('[data-card-grid] .deck-note')).toHaveCount(313);
  await expect(page.locator('[data-focus-dialog]')).toBeVisible();
