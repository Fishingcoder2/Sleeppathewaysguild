(function(){
  ...
  function buildTerms(payload,config){
    ...
    return (payload.cards||[]).filter(card=>included.has(card.category)).filter(card=>String(card.front||'').trim().length<=max).filter(card=>String(card.back||'').trim().length>=min).filter(card=>!config.excludeQuestionFronts||!looksLikeQuestion(String(card.front||'').trim())).map(card=>({id:card.id,front:String(card.front).trim(),back:String(card.back).trim(),category:card.category,memoryClue:card.memoryClue||''}));
  }
  ...
  async function init(){
    try{
      const [payload,config]=await Promise.all([libraryApi.load(),loadJson('data/memory/spg-glossary-config.json')]);
      terms=buildTerms(payload,config);
