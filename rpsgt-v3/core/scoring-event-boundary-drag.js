...
function toolbarMarkup(item){
  const id=caseId(item),mastered=state.mastered[id]===true,count=annotationsFor(item).length;
  return `<button class="btn secondary" type="button" data-boundary-drag-prev ${state.index===0?'disabled':''}>Previous</button><button class="btn secondary" type="button" data-boundary-drag-hint>${state.hints[id]?'Hide hint':'Hint'}</button>${mastered?`<button class="btn primary" type="button" data-boundary-drag-next>${state.index===state.cases.length-1?'Finish boundary-event skill':'Next'}</button>`:`<button class="btn primary" type="button" data-boundary-drag-check ${!count||state.confirming?'disabled':''}>Check event annotations</button>`}`;
}
function renderAnnotations(svg,item,preview){
  ...
  const spans=annotationsFor(item).map((span,index)=>({...span,index,preview:false}));
  if(preview&&Number.isInteger(preview.editIndex)&&preview.editIndex>=0&&spans[preview.editIndex])spans[preview.editIndex]={...preview,index:preview.editIndex,preview:true};else if(preview)spans.push({...preview,index:-1,preview:true});
  spans.forEach(span=>{
    const row=renderer.ROWS[span.channel]||renderer.ROWS.nasal,x=renderer.xFor(span.start),endX=renderer.xFor(span.end),width=Math.max(3,endX-x);
    const rect=document.createElementNS(ns,'rect');rect.setAttribute('x',String(x));rect.setAttribute('y',String(row-27));rect.setAttribute('width',String(width));rect.setAttribute('height','54');rect.setAttribute('rx','8');rect.setAttribute('class',`scoring-boundary-event-bar${span.preview?' preview':''}`);if(span.index>=0)rect.dataset.boundaryDragBar=String(span.index);group.appendChild(rect);
    const text=document.createElementNS(ns,'text');text.setAttribute('x',String(x+Math.max(20,width/2)));text.setAttribute('y',String(row-34));text.setAttribute('text-anchor','middle');text.setAttribute('class','scoring-boundary-event-bar-label');text.textContent=span.preview?(span.index>=0?'Adjust event boundary':'Drag to event end'):`Event ${span.index+1}`;group.appendChild(text);
    if(span.index>=0&&!state.mastered[caseId(item)]&&!state.confirming){
      [['start',x],['end',endX]].forEach(([edge,cx])=>{
        const hit=document.createElementNS(ns,'circle');hit.setAttribute('cx',String(cx));hit.setAttribute('cy',String(row));hit.setAttribute('r','15');hit.setAttribute('class','scoring-boundary-event-handle-hit');hit.dataset.boundaryDragHandle=edge;hit.dataset.boundaryDragIndex=String(span.index);group.appendChild(hit);
        const knob=document.createElementNS(ns,'circle');knob.setAttribute('cx',String(cx));knob.setAttribute('cy',String(row));knob.setAttribute('r','6');knob.setAttribute('class','scoring-boundary-event-handle');group.appendChild(knob);
      });
    }
  });svg.appendChild(group);
}
function refreshCaseChrome(){
  const item=currentCase();if(!item)return;const id=caseId(item),mastered=state.mastered[id]===true,count=annotationsFor(item).length,channel=selectedChannel(item);
  workspace.classList.toggle('tool-mark',state.tool==='mark');workspace.classList.toggle('tool-pan',state.tool==='pan');
  workspace.querySelectorAll('[data-boundary-drag-tool]').forEach(button=>button.classList.toggle('selected',button.dataset.boundaryDragTool===state.tool));
  workspace.querySelectorAll('[data-boundary-drag-channel]').forEach(button=>button.classList.toggle('selected',button.dataset.boundaryDragChannel===channel));
  const undo=workspace.querySelector('[data-boundary-drag-undo]'),clear=workspace.querySelector('[data-boundary-drag-clear]');if(undo)undo.disabled=!count||mastered||state.confirming;if(clear)clear.disabled=!count||mastered||state.confirming;
  const guide=workspace.querySelector('[data-boundary-drag-guide]');if(guide)guide.textContent=state.tool==='mark'?`Drag left-to-right on the ${channel==='thermal'?'Thermistor':'Nasal pressure'} row to place each event bar.`:'Swipe or drag horizontally to move through the strip.';
  const list=workspace.querySelector('[data-boundary-drag-list]');if(list)list.innerHTML=annotationMarkup(item);
  const toolbar=workspace.querySelector('[data-boundary-drag-toolbar]');if(toolbar)toolbar.innerHTML=toolbarMarkup(item);
  const feedback=workspace.querySelector('[data-boundary-drag-feedback]');if(feedback)feedback.innerHTML=`${confirmationMarkup(item)}${hintMarkup(item)}${feedbackMarkup(item)}`;
  const nav=workspace.querySelector('[data-boundary-drag-nav]');if(nav)nav.innerHTML=navMarkup();
}
function renderCase(){
  const item=currentCase();if(!item){finishSkill();return;}const id=caseId(item),channel=selectedChannel(item),count=annotationsFor(item).length;
  openModal();workspace.classList.toggle('tool-mark',state.tool==='mark');workspace.classList.toggle('tool-pan',state.tool==='pan');
  workspace.innerHTML=`<button class="scoring-boundary-modal-close" type="button" data-boundary-drag-close aria-label="Close event-boundary practice">×</button><div class="section-head"><div><div class="eyebrow">Phase 3 event annotation · Case ${state.index+1} of ${state.cases.length}</div><h3>${esc(item.title)}</h3></div><div class="scoring-boundary-status-stack"><span class="status">📌 Pinned until case complete</span><span class="status gold">First submission counts</span></div></div><div class="scoring-boundary-nav-row" data-boundary-drag-nav aria-label="Boundary cases">${navMarkup()}</div><p class="report-intro"><strong>Mark every respiratory event you see.</strong> Drag from event onset to event end directly across either airflow row. If you see two events, place two bars; if you see three, place three. The workspace stays pinned while you mark and adjust all events.</p><div class="scoring-boundary-rule"><strong>Display rule:</strong> The vertical 0:30 and 1:00 lines are epoch boundaries only. Follow the physiology through the line before deciding where an event ends.</div><div class="scoring-boundary-drag-tools"><div class="scoring-boundary-tool-group" aria-label="Tracing interaction"><button class="btn secondary ${state.tool==='mark'?'selected':''}" type="button" data-boundary-drag-tool="mark">Mark events</button><button class="btn secondary ${state.tool==='pan'?'selected':''}" type="button" data-boundary-drag-tool="pan">Pan strip</button></div><div class="scoring-boundary-tool-group" aria-label="Airflow annotation row"><button class="btn secondary ${channel==='nasal'?'selected':''}" type="button" data-boundary-drag-channel="nasal">Nasal pressure</button><button class="btn secondary ${channel==='thermal'?'selected':''}" type="button" data-boundary-drag-channel="thermal">Thermistor</button></div><button class="btn secondary" type="button" data-boundary-drag-undo ${!count?'disabled':''}>Undo last</button><button class="btn secondary" type="button" data-boundary-drag-clear ${!count?'disabled':''}>Clear all</button></div><div class="spg-rotate-guidance">Rotate your phone sideways for the widest tracing. Use Pan strip to move through the 90 seconds, then Mark events to drag annotations.</div><div class="scoring-boundary-visual spg-visual-surface" data-scoring-boundary-visual><button class="spg-visual-fullscreen-btn ${fullscreenSupported(workspace)?'':'is-unsupported'}" type="button" data-scoring-boundary-fullscreen>Full screen</button><div class="scoring-boundary-scroll-guide" data-boundary-drag-guide>${state.tool==='mark'?`Drag left-to-right on the ${channel==='thermal'?'Thermistor':'Nasal pressure'} row to place each event bar.`:'Swipe or drag horizontally to move through the strip.'}</div><div class="scoring-boundary-scroll" data-boundary-drag-scroll><svg class="scoring-boundary-svg" data-boundary-drag-svg></svg></div></div><p class="spg-visual-disclosure"><strong>AI-generated teaching schematic · Not a patient recording.</strong> Real PSG tracings vary. Compare authentic tracings in current sleep-technology textbooks, peer-reviewed educational resources, and official guidance.</p><div class="scoring-boundary-event-list" data-boundary-drag-list aria-label="Marked respiratory events">${annotationMarkup(item)}</div><div class="scoring-boundary-toolbar" data-boundary-drag-toolbar>${toolbarMarkup(item)}</div><div class="scoring-boundary-feedback" data-boundary-drag-feedback aria-live="polite">${confirmationMarkup(item)}${hintMarkup(item)}${feedbackMarkup(item)}</div>`;
...
function pointerDown(event){
  if(!state.active||state.tool!=='mark'||state.confirming)return;const svg=event.target.closest('[data-boundary-drag-svg]');if(!svg)return;const item=currentCase(),id=caseId(item);if(state.mastered[id])return;
  const handle=event.target.closest('[data-boundary-drag-handle]');
  if(handle){const index=Number(handle.dataset.boundaryDragIndex),edge=handle.dataset.boundaryDragHandle,list=annotationsFor(item),span=list[index];if(!span||!Number.isInteger(index)||(edge!=='start'&&edge!=='end'))return;event.preventDefault();event.stopImmediatePropagation();state.drag={mode:'edit',pointerId:event.pointerId,editIndex:index,edge,original:clone(span),preview:{...clone(span),editIndex:index}};if(svg.setPointerCapture)svg.setPointerCapture(event.pointerId);return;}
  const point=svgPoint(svg,event),channel=selectedChannel(item),row=renderer.ROWS[channel];if(!point||Math.abs(point.y-row)>34)return;const start=timeFromPoint(point);if(start==null)return;event.preventDefault();event.stopImmediatePropagation();state.drag={mode:'new',pointerId:event.pointerId,start,end:start,channel};if(svg.setPointerCapture)svg.setPointerCapture(event.pointerId);renderAnnotations(svg,item,state.drag);
}
function pointerMove(event){
  if(!state.drag||event.pointerId!==state.drag.pointerId)return;const svg=event.target.closest('[data-boundary-drag-svg]')||workspace.querySelector('[data-boundary-drag-svg]');if(!svg)return;const point=svgPoint(svg,event),time=timeFromPoint(point);if(time==null)return;event.preventDefault();
  if(state.drag.mode==='edit'){const preview={...state.drag.original,editIndex:state.drag.editIndex};if(state.drag.edge==='start')preview.start=Math.min(time,preview.end-.25);else preview.end=Math.max(time,preview.start+.25);state.drag.preview=preview;renderAnnotations(svg,currentCase(),preview);return;}
  state.drag.end=time;const preview={start:Math.min(state.drag.start,state.drag.end),end:Math.max(state.drag.start,state.drag.end),channel:state.drag.channel};renderAnnotations(svg,currentCase(),preview);
}
function pointerEnd(event){
  if(!state.drag||event.pointerId!==state.drag.pointerId)return;const item=currentCase(),id=caseId(item),svg=workspace.querySelector('[data-boundary-drag-svg]'),drag=state.drag;state.drag=null;
  if(drag.mode==='edit'){const preview=drag.preview||{...drag.original,editIndex:drag.editIndex};if(preview.end-preview.start>=2){annotationsFor(item)[drag.editIndex]={start:preview.start,end:preview.end,channel:preview.channel};state.feedback[id]=null;}if(svg)renderAnnotations(svg,item,null);refreshCaseChrome();return;}
  const span={start:Math.min(drag.start,drag.end),end:Math.max(drag.start,drag.end),channel:drag.channel};if(span.end-span.start>=2){annotationsFor(item).push(span);state.feedback[id]=null;if(svg)renderAnnotations(svg,item,null);refreshCaseChrome();return;}if(svg)renderAnnotations(svg,item,null);
}
function removeAnnotation(index){const item=currentCase(),id=caseId(item);if(!item||state.mastered[id]||state.confirming)return;const list=annotationsFor(item),target=Number(index);if(target>=0&&target<list.length){list.splice(target,1);state.feedback[id]=null;const svg=workspace.querySelector('[data-boundary-drag-svg]');if(svg)renderAnnotations(svg,item,null);refreshCaseChrome();}}
function askToCheck(){const item=currentCase();if(!item||!annotationsFor(item).length||state.mastered[caseId(item)])return;state.confirming=true;refreshCaseChrome();}
function submitCheck(){const item=currentCase();if(!item||!state.confirming)return;const id=caseId(item),selected=clone(annotationsFor(item));if(!state.firstResponses[id])state.firstResponses[id]=selected;const grade=greedyGrade(item,selected);state.confirming=false;state.mastered[id]=grade.correct;state.feedback[id]=grade;const svg=workspace.querySelector('[data-boundary-drag-svg]');if(svg)renderAnnotations(svg,item,null);refreshCaseChrome();}
...
function handleClick(event){
  ...
  const tool=event.target.closest('[data-boundary-drag-tool]');if(tool){event.preventDefault();event.stopImmediatePropagation();state.tool=tool.dataset.boundaryDragTool;refreshCaseChrome();return;}
  const channel=event.target.closest('[data-boundary-drag-channel]');if(channel){event.preventDefault();event.stopImmediatePropagation();state.channel[caseId(currentCase())]=channel.dataset.boundaryDragChannel;state.tool='mark';refreshCaseChrome();return;}
  ...
  if(event.target.closest('[data-boundary-drag-undo]')){event.preventDefault();event.stopImmediatePropagation();const item=currentCase(),list=annotationsFor(item);if(list.length)list.pop();state.feedback[caseId(item)]=null;const svg=workspace.querySelector('[data-boundary-drag-svg]');if(svg)renderAnnotations(svg,item,null);refreshCaseChrome();return;}
  if(event.target.closest('[data-boundary-drag-clear]')){event.preventDefault();event.stopImmediatePropagation();const item=currentCase();state.annotations[caseId(item)]=[];state.feedback[caseId(item)]=null;const svg=workspace.querySelector('[data-boundary-drag-svg]');if(svg)renderAnnotations(svg,item,null);refreshCaseChrome();return;}
  ...
  if(event.target.closest('[data-boundary-drag-change]')){event.preventDefault();event.stopImmediatePropagation();state.confirming=false;refreshCaseChrome();return;}
  ...
  if(event.target.closest('[data-boundary-drag-hint]')){event.preventDefault();event.stopImmediatePropagation();const id=caseId(currentCase());state.hints[id]=!state.hints[id];refreshCaseChrome();return;}
