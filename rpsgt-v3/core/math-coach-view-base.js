(function(root){
  ...
  app.configure=function(options){
    ...
    app.state={saved:null,catalog:null,currentSkill:null,guidedFeedback:null,setFeedback:null,returnFocus:null,guidedQuestionId:null,whiteboardOpen:false,whiteboardPinned:false,whiteboardQuestionId:null,whiteboardResponse:null,whiteboardKind:null,questionReviewKind:null};
  };
  ...
  app.questionById=function(skill,id){
    if(!skill||!id)return null;
    for(const group of ['guided','independent','mastery']){
      const match=(skill[group]||[]).find(question=>String(question.id)===String(id));
      if(match)return match;
    }
    return null;
  };
  app.resetQuestionHelp=function(){
    app.state.whiteboardOpen=false;app.state.whiteboardPinned=false;app.state.whiteboardQuestionId=null;app.state.whiteboardResponse=null;app.state.whiteboardKind=null;
  };
  ...
  app.stageHeader=function(skill,record,label){return `<div class="math-workspace-head"><div><span class="status gold">${app.esc(label)}</span><h2>${app.esc(skill.title)}</h2><p>${app.esc(skill.topic)}</p></div><button class="btn secondary" type="button" data-math-exit>${['guided','independent','mastery'].includes(record.stage)?'Close questions':'Exit'}</button></div><div class="math-stage-progress" aria-label="Math Coach learning stages">${app.engine.STAGES.slice(0,5).map((stage,index)=>`<span class="${stage===record.stage?'active':''} ${app.engine.STAGES.indexOf(record.stage)>index||record.status==='mastered'?'done':''}">${index+1}<small>${app.esc(app.stageLabel(stage))}</small></span>`).join('')}</div>`;};
