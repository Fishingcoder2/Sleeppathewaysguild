(function(root,factory){
  ...
  function validatePack(pack){
    ...
    questions.forEach(question=>{
      ...
      if(!['stage-choice','region-choice','point-click','interval-mark'].includes(question&&question.type))issues.push('unsupported-question-type:'+(question&&question.id||'unknown'));
  ...
  function buildSession(pack,seed){const validation=validatePack(pack);if(!validation.valid)throw new Error('Visual pack validation failed: '+validation.issues.join(', '));const studies=new Map(pack.studies.map(study=>[String(study.id),clone(study)])),studyOrder=shuffle(pack.studies.map(study=>String(study.id)),String(seed||'visual-session')),questionsByStudy=new Map(studyOrder.map(id=>[id,[]]));pack.questions.forEach(question=>{const id=String(question.studyId);if(!questionsByStudy.has(id))questionsByStudy.set(id,[]);questionsByStudy.get(id).push(clone(question));});return {id:'visual-'+String(seed||new Date().toISOString()),questions:studyOrder.flatMap(id=>questionsByStudy.get(id)||[]),studies,studyOrder};}
  ...
  function gradeSession(input){const questions=Array.isArray(input&&input.questions)?input.questions:[],answers=isObject(input&&input.answers)?input.answers:{},completedAt=input&&input.completedAt||new Date().toISOString(),responses=questions.map(question=>gradeAnswer(question,answers[String(question.id)])),correct=responses.filter(item=>item.correct).length,total=responses.length,percent=total?Math.round(correct/total*100):0;return {id:'visual-'+completedAt,source:'v3-lab-visual',labId:LAB_ID,correct,total,percent,completedAt,questionIds:questions.map(question=>question.id),responses};}
