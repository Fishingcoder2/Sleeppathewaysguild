(function(root,factory){
  ...
  function hashSeed(value){let hash=2166136261;const text=String(value||'visual');for(let i=0;i<text.length;i+=1){hash^=text.charCodeAt(i);hash=Math.imul(hash,16777619);}return hash>>>0;}
  ...
  function shuffle(values,seed){const result=values.slice(),random=seededRandom(seed);for(let i=result.length-1;i>0;i-=1){const j=Math.floor(random()*(i+1));const tmp=result[i];result[i]=result[j];result[j]=tmp;}return result;}
  ...
  function buildSession(pack,seed){
    ...
    const studyOrder=shuffle(pack.studies.map(study=>String(study.id)),String(seed||'visual-session'));
    const questionsByStudy=new Map(studyOrder.map(id=>[id,[]]));
    pack.questions.forEach(question=>{const id=String(question.studyId);if(!questionsByStudy.has(id))questionsByStudy.set(id,[]);questionsByStudy.get(id).push(clone(question));});
    const questions=studyOrder.flatMap(id=>questionsByStudy.get(id)||[]);
    return {id:'visual-'+String(seed||new Date().toISOString()),questions,studies,studyOrder};
  }
  ...
  function normalizeRecord(value){
    const source=isObject(value)?value:{},history=Array.isArray(source.history)?source.history.filter(isObject).map(clone):[];
    return {status:source.status==='in-progress'?'in-progress':'not-started',startedAt:source.startedAt||null,updatedAt:source.updatedAt||null,attempts:Math.max(history.length,safeNumber(source.attempts,history.length)),bestPercent:Math.max(0,Math.min(100,safeNumber(source.bestPercent,0))),latestSession:isObject(source.latestSession)?clone(source.latestSession):history[0]||null,history};
  }
  ...
  return {VERSION,LAB_ID,HISTORY_LIMIT,hashSeed,shuffle,parsePoint,pointAnswer,validatePack,buildSession,gradeAnswer,gradeSession,normalizeLabs,start,applySession,summary};
});
