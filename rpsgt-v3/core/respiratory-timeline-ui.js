(function(){
  ...
  function eventContains(event,t){return Boolean(event)&&t>=event.start&&t<=event.end;}
  ...
  function centralEnvelope(t,cycle,irregular){
    ...
    let value=0.04+0.96*Math.pow((1+Math.sin(phase))/2,0.88);
    if(value<0.15) value=0.025;
    return clamp(value,0.02,1);
  }

  function longSignal(def,channel,t){
    if(channel==='eeg'){
      let value=1.25*Math.sin(t*3.7)+0.8*Math.sin(t*8.4+0.7)+0.55*Math.sin(t*15.2+0.2)+0.35*Math.sin(t*23.1);
    ...
    const phase=channel==='abdomen'?0.12:(channel==='thermal'?0.06:0);
    const value=breath(t,phase);
    const amplitude=channel==='thermal'?0.8:1;
    if(def.kind==='stable') return value*amplitude;
    if(def.kind==='cheyne-stokes') return value*amplitude*centralEnvelope(t,def.cycleSeconds,false);
    if(def.kind==='periodic-breathing'){
      const env=centralEnvelope(t,def.cycleSeconds,true);
      const irregular=clamp(env*(0.9+0.12*Math.sin(t/11)+0.06*Math.sin(t/5.3)),0.025,1);
    ...
    if(def.kind==='recurrent-central') return value*0.025;
    if(channel==='nasal'||channel==='thermal') return value*0.025;
    const progress=intervalProgress(t,events);
    if(channel==='thorax') return value*(1.15+0.7*progress);
    if(channel==='abdomen') return breath(t,Math.PI)*(1.18+0.65*progress);
    return value;
  }
  ...
  function eventSignal(def,channel,t){
    ...
    if(def.pattern==='obstructive-apnea'){
      if(channel==='nasal'||channel==='thermal') return wave*0.02;
      if(channel==='thorax') return wave*(1.2+0.7*progress);
      if(channel==='abdomen') return breath(t,Math.PI)*(1.2+0.62*progress);
    }
    if(def.pattern==='central-apnea') return wave*0.02;
    if(def.pattern==='mixed-apnea'){
      if(channel==='nasal'||channel==='thermal') return wave*0.02;
      const transition=def.transition||((event.start+event.end)/2);
      if(t<transition) return wave*0.02;
      const resumed=clamp((t-transition)/(event.end-transition),0,1);
      if(channel==='thorax') return wave*(1.15+0.6*resumed);
      if(channel==='abdomen') return breath(t,Math.PI)*(1.12+0.6*resumed);
    }
    if(def.pattern==='obstructive-hypopnea'){
      if(channel==='nasal'){
        wave=wave>0?Math.min(wave,0.26):wave*0.84;
        return wave*0.48;
      }
      if(channel==='thermal') return wave*0.78;
      if(channel==='thorax') return wave*(1.15+0.4*progress);
      if(channel==='abdomen') return breath(t,Math.PI)*(1.13+0.38*progress);
    }
    if(def.pattern==='central-hypopnea'){
      if(channel==='nasal') return wave*0.46;
      if(channel==='thermal') return wave*0.64;
      if(channel==='thorax'||channel==='abdomen') return wave*0.43;
    }
    if(def.pattern==='rera'){
      if(channel==='nasal'){
        wave=wave>0?Math.min(wave,0.28):wave*0.88;
        return wave*(0.72-0.18*progress);
      }
      if(channel==='thermal') return wave*0.9;
      if(channel==='thorax') return wave*(1.05+0.42*progress);
      if(channel==='abdomen') return wave*(1.03+0.38*progress);
    }
  ...
  function delayedDrop(t,event,depth){
    if(!event) return 0;
    const start=event.end+6;
    const nadir=event.end+24;
    const recovery=event.end+52;
  ...
  function samplePath(def,channel,duration){
    if(channel==='spo2') return spo2Path(def,duration);
  ...
  function spo2Path(def,duration){
    const parts=[];
    const count=duration===300?500:300;
    for(let index=0;index<count;index+=1){
      const t=duration*index/(count-1);
      const x=PLOT.left+(PLOT.right-PLOT.left)*index/(count-1);
  ...
  function oxygenLabels(def,duration){
    return engine.timelineTicks(duration,30).map(seconds=>{
      ...
      return `<text class="resp-timeline-spo2-number" x="${x.toFixed(1)}" y="${ROWS.spo2-15}" text-anchor="middle">${sat}</text>`;
    }).join('');
  }
  ...
  function targetHighlight(task,duration){
  ...
  function traceSvg(def,duration,interactive,task){
    ...
    const guides=Object.entries(ROWS).map(([,y])=>`<line x1="${PLOT.left}" y1="${y}" x2="${PLOT.right}" y2="${y}"></line>`).join('');
  ...
  function timeTargetButtons(item,task){
    ...
    for(let start=0;start<item.duration;start+=30){
      const end=Math.min(item.duration,start+30);
  ...
  function renderEvidence(){
    ...
    if(!item||!task) return;
    const progress=completedBefore()+1;
    const total=totalEvidenceTasks();
  ...
  function handleGuess(channel,time){
    ...
    if(result.correct){state.solved=true;state.correct+=1;state.feedback=task.explanation;}
    else{state.misses+=1;state.feedback=`Not there yet. ${task.hint}`;}
    renderEvidence();
  }
