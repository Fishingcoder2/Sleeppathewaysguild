(function(){
  ...
  const state={saved:null,blueprint:null,index:null,catalog:null,summary:null};
  ...
  function rebuild(){state.summary=engine.buildSummary({saved:state.saved,blueprint:state.blueprint,questionIndex:state.index,catalog:state.catalog,includeLearnerName:includeName(),generatedAt:new Date().toISOString()});render();}
  ...
  function renderPlan(summary){const host=$('[data-summary-plan]');host.innerHTML=summary.studyPlan.length?summary.studyPlan.map((item,index)=>'<article class="summary-plan-item"><span class="summary-rank">'+(index+1)+'</span><div><h3>'+esc(item.taskCode)+' · '+esc(item.title)+'</h3><p>'+esc(item.practiceCorrect)+' / '+esc(item.practiceAnswered)+' practice correct · '+esc(item.practiceAccuracy)+'% · '+esc(item.missed)+' currently missed</p>'+(item.topics.length?'<p><strong>Topics:</strong> '+item.topics.map(topic=>esc(topic.label)+' ('+esc(topic.count)+')').join(', ')+'</p>':'')+'<p><strong>Next move:</strong> '+esc(item.nextAction)+'</p></div></article>').join(''):'<div class="summary-empty">Complete Practice, Readiness, or Mock work to generate a prioritized study plan.</div>';}
  ...
  async function init(){try{
    ...
    [state.blueprint,state.index,state.catalog]=await Promise.all([loadJson('data/blueprint.json'),loadJson('data/question-bank/feedback-index.json'),loadJson('data/labs/catalog.json')]);
