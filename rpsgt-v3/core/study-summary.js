(function(){
  ...
  function resourceHtml(item){
    if(!item.resources||!item.resources.length)return '<div class="summary-resource-empty">No verified section-level resource route is mapped for this priority yet.</div>';
    return '<div class="summary-resource-route"><strong>Mapped study resources</strong><p class="summary-resource-note">Selected from question interactions, weak-task evidence, and the verified task/topic crosswalk. Official rule sources are listed before supporting texts when available.</p>'+item.resources.map((group,index)=>'<div class="summary-resource-group"><div><span class="summary-resource-order">'+(index+1)+'</span><div><strong>'+esc(group.sourceTitle)+'</strong><small>'+esc(group.sourceType)+(group.bestFor?' · '+esc(group.bestFor):'')+'</small></div></div><ul>'+group.sections.map(section=>'<li>'+esc(section.label)+(section.reason?'<small>'+esc(section.reason)+'</small>':'')+'</li>').join('')+'</ul></div>').join('')+'</div>';
  }
