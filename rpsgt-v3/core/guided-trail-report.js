(function(){
  ...
  async function render(){
    ...
    try{
      ...
      family.innerHTML=`<strong>${counts.taskAwards} task badges · ${counts.domainAwards} domain medals</strong><span>${counts.studyMarks}/12 study marks · ${counts.taskAwards}/12 task badges · ${counts.domainAwards}/4 domain medals · ${counts.checkpoints} checkpoints</span>`;
      const latest=report.latestCheckpoint;
      const domainRows=report.domains.map(domain=>{const medal=DOMAIN_MEDALS[domain.id]||`${domain.id} domain medal`;return `<article class="trail-domain-row"><div><strong>${esc(domain.id)} · ${esc(domain.name)}</strong><small>${domain.studyMarked}/${domain.taskCount} study marks · ${domain.taskAwards}/${domain.taskCount} task badges</small></div><span class="status ${domain.award?'green':'gold'}">${domain.award?`${esc(medal)} medal earned`:`Toward ${esc(medal)}`}</span></article>`;}).join('');
      ...
      host.innerHTML=`<div class="trail-report-summary"><div><span>Study marks</span><strong>${counts.studyMarks}/12</strong></div><div><span>Task badges</span><strong>${counts.taskAwards}/12</strong></div><div><span>Domain medals</span><strong>${counts.domainAwards}/4</strong></div><div><span>Checkpoint attempts</span><strong>${counts.checkpoints}</strong></div></div><div class="report-callout"><strong>Current position:</strong> ${report.currentFocus?esc(report.currentFocus.task||report.currentFocus.domain):'No Guided Study task selected yet.'}${latest?` · Latest checkpoint ${esc(latest.task)} ${esc(latest.score)}%`:''}</div><div class="trail-domain-list">${domainRows}</div><p class="report-intro"><strong>Achievement boundary:</strong> Task badges and named domain medals are Sleep Pathways Guild educational achievements. They are not BRPT-issued credentials, official exam scores, or passing predictions.</p><h3>Recent checkpoint history</h3>${history?`<div class="history-list">${history}</div>`:'<div class="empty-report">No Guided Trail checkpoint has been completed yet.</div>'}`;
