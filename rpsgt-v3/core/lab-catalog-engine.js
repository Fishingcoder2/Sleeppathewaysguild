(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;root.RPSGTLabCatalogEngine=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function hookupCompleted(source){
    const record=isObject(source&&source.hookup)?source.hookup:null;
    return Boolean(record&&Number(record.skillVersion)>=2&&record.skillsCompleted===true&&record.quizPassed===true);
  }

  function normalizeProgress(value){
    const source=isObject(value)?value:{};
    const completed=new Set(Array.isArray(source.completed)?source.completed.map(String):[]);
    ...
    return {
      ...
      started,
      lastLab:typeof source.lastLab==='string'?source.lastLab:null,
      catalogIndex:Number.isFinite(Number(source.catalogIndex))?Number(source.catalogIndex):null,
      raw:clone(source)
    };
  }

  function validateCatalog(catalog){
    const labs=Array.isArray(catalog&&catalog.labs)?catalog.labs:[];
    const ids=new Set();
    const issues=[];
    labs.forEach((lab,index)=>{
      if(!lab||typeof lab.id!=='string'||!lab.id) issues.push({code:'missing-lab-id',index});
      else if(ids.has(lab.id)) issues.push({code:'duplicate-lab-id',id:lab.id});
      else ids.add(lab.id);
      if(!Array.isArray(lab.taskCodes)||!lab.taskCodes.length) issues.push({code:'missing-task-map',id:lab&&lab.id});
      if(!['catalog-only','legacy-linked','v3-ready'].includes(lab&&lab.status)) issues.push({code:'invalid-status',id:lab&&lab.id});
      if(lab&&lab.status==='legacy-linked'&&!lab.legacyHref) issues.push({code:'missing-legacy-link',id:lab.id});
    });
