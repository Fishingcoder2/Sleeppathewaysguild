(function(){
  ...
  function renderSummary(){
    ...
    summaryHost.innerHTML=`<div><span>Status</span><strong>${report.completed?'Review complete':report.status==='in-progress'?'Review in progress':'Not started'}</strong></div><div><span>Review steps</span><strong>${report.stationsComplete}/${report.stationCount}</strong></div><div><span>Best checkpoint</span><strong>${report.attempts?report.bestPercent+'%':'—'}</strong></div><div><span>Attempts</span><strong>${report.attempts}</strong></div><div><span>Last checkpoint</span><strong>${report.latestSession?formatDate(report.latestSession.completedAt):'—'}</strong></div>`;
  ...
  function renderSession(){
    ...
    workspace.innerHTML=`<div class="section-head"><div><div class="eyebrow">D2B · D3C EKG checkpoint</div><h2>Ten EKG workflow questions</h2></div><button class="btn secondary" type="button" data-ekg-cancel>Close checkpoint</button></div><p class="report-intro">Use these original learner questions to rehearse signal validity, systematic rhythm review, patient-first response, escalation boundaries, and documentation. Correctness and reasoning are shown after the checkpoint is scored.</p><form data-ekg-form>${state.questions.map((question,index)=>`<fieldset class="ekg-question"><legend><span>${index+1}</span>${esc(question.prompt)}</legend>${question.options.map(option=>`<label><input type="radio" name="ekg-${esc(question.id)}" value="${esc(option)}"> <span>${esc(option)}</span></label>`).join('')}</fieldset>`).join('')}<div class="actions"><button class="btn primary" type="submit">Score EKG checkpoint</button></div></form><div data-ekg-result></div>`;
  ...
  function renderResult(record){
    ...
    const report=engine.summary(state.saved.labs);
    host.innerHTML=`<div class="ekg-result ${record.passed?'pass':'retry'}"><h3>${report.completed?'EKG review complete':record.passed?'Checkpoint passed — finish the review steps':'Checkpoint saved — review and retry'}</h3><strong>${record.correct}/${record.total} correct · ${record.percent}%</strong><p>${report.completed?'All seven review steps and the checkpoint milestone are complete. This remains study-review evidence, not demonstrated skill completion.':record.passed?'The 80% checkpoint milestone is complete. Finish every review step to finish this study review.':'An 80% score is required for the current review milestone. Your best score and attempt history remain available.'}</p></div><h3>Answer review</h3>${review}`;
  }
  function startSession(){
    ...
    if(state.questions.length<engine.SESSION_SIZE){workspace.hidden=false;workspace.innerHTML='<div class="notice error"><strong>EKG checkpoint unavailable.</strong> Fewer than ten eligible learner questions were found.</div>';return;}
  ...
  async function init(){
    try{
      if(!engine||!window.RPSGTStorage)throw new Error('A required EKG review module is unavailable.');
      ...
      if(!supplement||!supplement.meta||supplement.meta.appAuthored!==true||!Array.isArray(supplement.questions)||supplement.meta.questionCount!==supplement.questions.length)throw new Error('The EKG question supplement could not be verified.');
      state.bank=[...(d2b.questions||[]),...(d3c.questions||[]),...supplement.questions];
      if(engine.eligibleQuestions(state.bank).length<engine.SESSION_SIZE)throw new Error('The EKG question library does not contain enough eligible learner questions.');
      renderSummary();renderStations();
    }catch(error){workspace.hidden=false;workspace.innerHTML=`<div class="notice error"><strong>EKG review could not load.</strong> ${esc(error.message)} No learner progress was changed.</div>`;startButton.disabled=true;}
  }
