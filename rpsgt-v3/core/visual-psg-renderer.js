(function(root,factory){
  ...
  function sample(channel,t){
    ...
    if(channel.type==='eog'){
      let base=.08*deterministicNoise(t,phase);
      if(channel.profile==='slow-eye')base+=polarity(channel)*.82*Math.sin(TAU*.20*t+phase);
      else if(channel.profile==='wake')base+=polarity(channel)*.20*Math.sin(TAU*.12*t+phase);
      else if(channel.profile==='rem')base+=polarity(channel)*.08*Math.sin(TAU*.16*t+phase);
      else base+=.08*Math.sin(TAU*.12*t+phase);
