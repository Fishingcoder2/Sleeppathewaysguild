(function(){
  ...
  const state={catalog:null,saved:null,answers:{},index:0,result:null};
  ...
  function renderHome(){
    ...
    home.hidden=false;sessionHost.hidden=true;resultsHost.hidden=true;
  }
  function start(){
    state.answers={};state.index=0;state.result=null;
  ...
  function renderQuestion(){
    home.hidden=true;resultsHost.hidden=true;sessionHost.hidden=false;
    ...
    sessionHost.innerHTML=`<div class="md-question-number"><div><span class="status">Question ${state.index+1} of ${total}</span> <span class="status green">${esc(question.taskCode)}</span> <span class="status gold">${esc(section.title||question.section)}</span></div><strong>${answered}/${total} answered</strong></div>
      <div class="md-progress" aria-label="Question progress"><span style="width:${Math.round((state.index+1)/total*100)}%"></span></div>
      <div class="md-question">${esc(question.prompt)}</div>
      ...
      <div class="md-palette" aria-label="Question navigator">${state.catalog.questions.map((item,index)=>`<button type="button" data-md-jump="${index}" class="${state.answers[item.id]!==undefined?'answered':''} ${index===state.index?'current':''}" aria-label="Go to question ${index+1}">${index+1}</button>`).join('')}</div>
      <div class="md-nav"><button class="btn secondary" type="button" data-md-prev ${state.index===0?'disabled':''}>Previous</button><div class="actions">${state.index<total-1?'<button class="btn primary" type="button" data-md-next>Next</button>':'<button class="btn primary" type="button" data-md-submit>Submit diagnostic</button>'}</div></div>
      <p class="md-boundary">Answers are saved only when the diagnostic is submitted. No correctness feedback is shown during the run.</p>`;
    sessionHost.scrollIntoView({behavior:'smooth',block:'start'});
  }
  function choose(value){const question=current();state.answers[question.id]=Number(value);renderQuestion();}
  function submit(){
    ...
    state.result=engine.grade(state.catalog,state.answers,new Date().toISOString());
  ...
  async function copyPlan(text,button){
    try{await navigator.clipboard.writeText(text);const old=button.textContent;button.textContent='Copied';setTimeout(()=>button.textContent=old,1400);}catch(error){window.prompt('Copy this mentoring plan:',text);}
  }
  function renderResults(){
    ...
    sessionHost.hidden=true;home.hidden=true;resultsHost.hidden=false;resultsHost.scrollIntoView({behavior:'smooth',block:'start'});
  }
  async function init(){
    try{
  ...
  document.addEventListener('click',event=>{
    if(event.target.closest('[data-md-start]')){start();return;}
