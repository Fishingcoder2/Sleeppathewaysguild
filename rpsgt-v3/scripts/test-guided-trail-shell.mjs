...
const [studyHtml,studyJs,presentationJs,surfaceGuard,reportsHtml,reportJs]=await Promise.all([
  readFile(join(root,'study.html'),'utf8'),readFile(join(root,'core','study.js'),'utf8'),readFile(join(root,'core','study-presentation.js'),'utf8'),readFile(join(root,'core','learner-surface-guard.js'),'utf8'),readFile(join(root,'reports.html'),'utf8'),readFile(join(root,'core','guided-trail-report.js'),'utf8')
]);
...
for(const script of ['core/learner-surface-guard.js','core/guided-trail-engine.js','core/study.js','core/study-presentation.js']) if(!studyHtml.includes(script)) throw new Error(`Guided Study page does not load ${script}.`);
...
console.log('Guided Study study/report shell, learner surface, source-key suppression, and read-only contracts passed.');
