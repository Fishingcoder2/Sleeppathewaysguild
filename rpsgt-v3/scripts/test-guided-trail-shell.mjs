...
for(const selector of ['data-guided-trail-dashboard','data-checkpoint-workspace','data-blueprint-map']) if(!studyHtml.includes(selector)) throw new Error(`Guided Study page is missing ${selector}.`);
...
for(const token of ['data-trail-mark','data-checkpoint-start','gradeCheckpoint','applyCheckpoint']) if(!studyJs.includes(token)) throw new Error(`Guided Study controller is missing ${token}.`);
