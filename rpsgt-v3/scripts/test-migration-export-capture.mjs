...
assert.ok(html.includes('name="robots" content="noindex,nofollow"'),'The private migration utility must remain noindex/nofollow.');
...
assert.equal(index.includes('migration-export.html'),false,'Learner dashboard must not expose the private migration capture utility.');assert.ok(ignore.includes('*')&&ignore.includes('!README.md'));assert.ok(privateReadme.includes('Do not commit'));
...
console.log('Read-only internal browser capture page, learner-navigation exclusion, recognized-key filtering, private-file guard, envelope metadata, and raw-value-free summary contracts passed.');
