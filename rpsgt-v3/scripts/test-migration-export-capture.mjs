...
assert.ok(index.includes('migration-export.html'),'Dashboard must expose the private migration capture utility.');assert.ok(ignore.includes('*')&&ignore.includes('!README.md'));assert.ok(privateReadme.includes('Do not commit'));
...
console.log('Read-only browser capture page, recognized-key filtering, private-file guard, envelope metadata, and raw-value-free summary contracts passed.');
