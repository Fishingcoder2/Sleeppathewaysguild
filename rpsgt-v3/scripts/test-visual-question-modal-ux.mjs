...
const [visualHtml,visualNav,visualNavCss,practiceHtml,reviewHtml,reviewJs]=await Promise.all([
...
for(const token of ['core/visual-navigation.js','assets/visual-navigation.css','data-visual-workspace']) if(!visualHtml.includes(token)) throw new Error(`Visual page is missing ${token}.`);
for(const token of ['visual-modal-open','visual-modal-active','visual-modal-footer','Rotate your phone sideways','Previous visual','Previous epoch','Next epoch','Check answer','Next visual','data-visual-modal-close','aria-modal']) if(!visualNav.includes(token)) throw new Error(`Visual modal navigation is missing ${token}.`);
