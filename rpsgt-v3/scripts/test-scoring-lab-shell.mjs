...
for(const style of ['.scoring-multi-sequence','.scoring-multi-epoch-card','.scoring-multi-trace','.scoring-multi-stage-options','.multi-correct','.multi-wrong','overflow-x:auto']) if(!multiCss.includes(style)) throw new Error(`Scoring consecutive-epoch CSS is missing ${style}.`);
