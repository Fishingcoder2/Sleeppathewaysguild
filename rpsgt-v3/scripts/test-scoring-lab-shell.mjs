...
for(const token of ['data-scoring-station-open','data-scoring-station-complete','Complete station &amp; continue','function completeStation','function nextStation','function previousStation']) if(!js.includes(token)) throw new Error(`Scoring guided review walkthrough is missing ${token}.`);
if(/type="checkbox" data-scoring-station/.test(js)) throw new Error('Scoring review must not return to a manually checked checklist.');
...
if(!html.includes('do not reproduce or replace proprietary AASM scoring-manual text')||!html.includes('five-epoch staging skill')||!html.includes('all seven review stations')||!html.includes('sixteen supporting clues')||!html.includes('AASM Scoring Manual Version 3')||!html.includes('Clinical atlas of polysomnography')||!html.includes('Phase 3 consecutive-epoch practice is currently tracked separately')) throw new Error('Scoring evidence, authority, visual corroboration, and completion boundaries must be visible.');
if(!html.includes('Seven-station guided review')||!html.includes('records each station when you finish its walk-through and continue')) throw new Error('Scoring page must explain the auto-recording guided review flow.');

for(const style of ['.scoring-stage-trace','.scoring-stage-options','.stage-correct','.stage-wrong','.scoring-event-trace','.scoring-event-options','.event-correct','.event-wrong','.scoring-event-highlight','overflow-x:auto','.scoring-walkthrough','.scoring-walkthrough-progress','.scoring-walkthrough-step','.scoring-walkthrough-card']) if(!css.includes(style)) throw new Error(`Scoring skill CSS is missing ${style}.`);
