...
for(const token of ['data-scoring-boundary-svg','data-scoring-boundary-count','data-scoring-boundary-mode','data-scoring-boundary-time','gradeEventBoundarySkill','applyEventBoundarySkill','First checked response counts']) if(!controller.includes(token)) throw new Error(`Event-boundary controller is missing ${token}.`);
...
for(const token of ['overflow-x:auto','-webkit-overflow-scrolling:touch','touch-action:pan-x pan-y','.scoring-boundary-divider','.scoring-boundary-marker','.scoring-boundary-target']) if(!css.includes(token)) throw new Error(`Event-boundary CSS is missing ${token}.`);
...
console.log('Scoring Phase 3 event-boundary practice passed: continuous 90-second physiology, three display epochs, boundary-crossing targets, 1/2/3-event counting, start/end placement, 80% grading, persistence, mobile horizontal scrolling, and unchanged completion boundary are present.');
