...
for(const token of ['.scoring-boundary-drag-tools','.tool-mark .scoring-boundary-svg{touch-action:none','.tool-pan .scoring-boundary-svg{touch-action:pan-x pan-y','.scoring-boundary-event-bar','.scoring-boundary-event-chip','.scoring-boundary-confirmation{position:fixed!important','orientation:landscape']) if(!dragCss.includes(token)) throw new Error(`PSG-style event annotation CSS is missing ${token}.`);

console.log('Scoring Phase 3 event-boundary practice passed: continuous 90-second physiology, three display epochs, PSG-style drag annotations on nasal-pressure or thermistor rows, 1/2/2/3 event-bar cases totaling eight scored first-pass annotations, 7/8 pass math, correction mastery, popout confirmation, hint, Previous/Next, fullscreen, AI disclosure, persistence, pan-versus-mark mobile interaction, and unchanged completion boundary are present.');
