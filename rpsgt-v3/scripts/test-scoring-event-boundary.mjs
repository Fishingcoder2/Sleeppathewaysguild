...
if(pack.cases.map(item=>item.events.length).join(',')!=='1,2,2,3') throw new Error('Event-boundary pack must exercise one-, two-, and three-reduction annotation.');
const totalEvents=pack.cases.reduce((sum,item)=>sum+item.events.length,0);
if(totalEvents!==8) throw new Error(`Event-boundary pack must contain eight total candidate reduction spans; found ${totalEvents}.`);
if(!/placement only/i.test(pack.meta.clinicalBoundary)||!/not presented as fully classified apneas or hypopneas/i.test(pack.meta.clinicalBoundary)) throw new Error('Boundary pack must remain explicitly classification-neutral placement practice.');
for(const item of pack.cases){
  ...
  const learnerText=`${item.title} ${item.prompt} ${item.focus} ${item.rationale}`;
  if(/obstructive apnea|central apnea|hypopnea|rera/i.test(learnerText)) throw new Error(`${item.id} must not pre-classify a generic candidate reduction.`);
}
...
for(const token of ['one continuous 90-second','does not turn one continuous physiologic reduction into two','4 cases · 8 annotations','If there are two reductions, place two bars','Seven of the eight first-pass annotations']) if(!html.includes(token)) throw new Error(`Learner-facing PSG-style boundary-placement explanation is missing: ${token}`);
if(!html.includes('candidate airflow reduction')||!html.includes('placement—not whether each generic reduction independently satisfies all Version 3 apnea or hypopnea criteria')) throw new Error('Learner-facing boundary skill must explicitly separate placement from Version 3 event classification.');
...
console.log('Scoring Phase 3 event-boundary practice passed: continuous 90-second physiology, three display epochs, classification-neutral candidate-reduction placement, pinned PSG-style drag annotations on nasal-pressure or thermistor rows, adjustable start/end handles, no modal rebuild after each drag, 1/2/2/3 bars totaling eight scored first-pass annotations, 7/8 pass math, correction mastery, popout confirmation, hint, Previous/Next, fullscreen, AI disclosure, persistence, pan-versus-mark mobile interaction, and unchanged completion boundary are present.');
