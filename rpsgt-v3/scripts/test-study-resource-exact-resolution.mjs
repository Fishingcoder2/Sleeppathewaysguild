...
for(const key of ['aasm-mslt-mwt','aasm-pap-titration','pediatric-psg','principles-practice-pediatric-sleep']){
  if(!Object.prototype.hasOwnProperty.call(aliases.intentionallyUnresolved||{},key)) throw new Error(`${key} must remain intentionally unresolved until exact provenance is verified.`);
}
...
try{
  ...
  if(!ambiguous.unresolvedKeys.includes('aasm-mslt-mwt')) throw new Error('Ambiguous AASM MSLT/MWT shorthand was not preserved as unresolved evidence.');
  ...
  console.log(JSON.stringify({
    ...
    unresolvedKeysExposedForQa:true
  },null,2));
}finally{
