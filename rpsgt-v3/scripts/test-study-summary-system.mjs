...
const [blueprint,index,catalog]=await Promise.all([
...
const saved={
  ...
  progress:{answered:20,correct:14,byTask:{D3A:{answered:10,correct:6},D4A:{answered:10,correct:8}},history:[{questionId:d3a.id},{questionId:d4a.id}]},
  review:{missedIds:[d3a.id],masteredIds:[d4a.id],flaggedIds:[d3a.id]},
  readiness:{history:[
    {completedAt:'2026-08-03T10:00:00.000Z',size:50,answered:50,correct:20,percent:40,weightedPercent:42,weakestTasks:[{taskCode:'D3A',title:'Adult scoring',total:10,correct:4,missed:6,percent:40,topics:['staging']}]},
  ...
  mock:{history:[
    ...
    {version:2,completedAt:'2026-08-03T12:00:00.000Z',timed:true,elapsedMs:7200000,answeredTotal:175,scoredCount:150,pretestCount:25,scoredCorrect:120,scoredPercent:80,weightedPercent:83,unansweredCount:0,weakestTasks:[{taskCode:'D4A',title:'PAP',total:15,correct:8,missed:7,percent:53}],details:[{id:d4a.id,flagged:true,selectedIndex:1,correct:false,role:'scored'}]}
  ]},
...
const before=JSON.stringify(saved);const summary=engine.buildSummary({saved,blueprint,questionIndex:index,catalog,generatedAt:'2026-08-03T15:00:00.000Z'});
...
if(summary.labs.counts.total!==10||summary.labs.counts.completed!==1||summary.labs.counts.started!==1)throw new Error('Laboratory summary failed.');
if(!summary.studyPlan.length||summary.studyPlan[0].taskCode!=='D3A')throw new Error('Combined study-plan ranking failed.');
...
const named=engine.buildSummary({saved,blueprint,questionIndex:index,catalog,generatedAt:'2026-08-03T15:00:00.000Z',includeLearnerName:true});if(named.learner.displayName!=='Private Learner Name'||named.privacy.learnerNameIncluded!==true)throw new Error('Opt-in learner-name inclusion failed.');
const csv=engine.toCsv(summary);if(!csv.startsWith('section,code,title,metric,value,detail'))throw new Error('CSV header failed.');if(!csv.includes('practice-task,D3A')||!csv.includes('lab,hookup'))throw new Error('CSV task or laboratory rows are missing.');for(const forbidden of ['Private Learner Name','PRIVATE NOTE SENTINEL','PRIVATE SEARCH SENTINEL','drive.google.com','PRIVATE QUESTION PROMPT SENTINEL'])if(csv.includes(forbidden))throw new Error(`CSV leaked forbidden content: ${forbidden}`);
if(JSON.stringify(saved)!==before)throw new Error('Study summary engine mutated its source learner record.');
console.log('Study summary engine passed privacy, ordering, task, diagnostic, Trail, laboratory, CSV, learner-name opt-in, and source-immutability contracts.');
