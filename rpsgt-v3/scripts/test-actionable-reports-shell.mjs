...
for(const phrase of ['plain-language note from Coach Bob','Your improvement roadmap','Study materials to use','Practice inside the webapp','practice.html?task='])if(!reportsAction.includes(phrase))throw new Error('Reports Center actionable recommendation UI is missing: '+phrase);
...
for(const phrase of ['Guided Study: ','Focused Practice: ','Review missed questions','Recheck after remediation','Math Coach','Memory Games','Flashcard Center'])if(!engine.includes(phrase))throw new Error('Improvement engine is missing an in-app remediation route: '+phrase);
