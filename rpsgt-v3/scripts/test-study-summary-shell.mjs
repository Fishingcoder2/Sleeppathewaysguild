...
const [html,controller,engine,css,resourceCss,reports]=await Promise.all([
...
if(!html.includes('assets/study-summary-resources.css'))throw new Error('Related-resource print stylesheet is missing.');
...
for(const phrase of ['not an official BRPT examination score','question text','answer text','rationales','notes','searches','private links','raw browser storage','related study resources'])if(!html.toLowerCase().includes(phrase.toLowerCase()))throw new Error(`Study summary privacy, disclaimer, or recommendation text is missing: ${phrase}`);
...
if(!controller.includes('loadStudySources')||!controller.includes('Related study resources'))throw new Error('Printable summary is not loading or rendering related study resources.');
...
if(!css.includes('@media print')||!css.includes('@page')||!css.includes('.no-print')||!css.includes('break-before:page')||!resourceCss.includes('@media print')||!resourceCss.includes('.summary-resource-group'))throw new Error('Print stylesheet contract is incomplete.');
...
console.log('Printable study summary page, privacy boundary, interaction-based related resources, learner-name default, read-only controller, JSON/CSV downloads, print CSS, script order, and Reports route contracts passed.');
