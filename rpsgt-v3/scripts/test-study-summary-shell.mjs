...
if(!html.includes('assets/study-summary-resources.css'))throw new Error('Mapped-resource print stylesheet is missing.');
...
for(const phrase of ['not an official BRPT examination score','question text','answer text','rationales','notes','searches','private links','raw browser storage','mapped study resources'])if(!html.toLowerCase().includes(phrase.toLowerCase()))throw new Error(`Study summary privacy, disclaimer, or recommendation text is missing: ${phrase}`);
...
if(!controller.includes('loadStudySources')||!controller.includes('Mapped study resources'))throw new Error('Printable summary is not loading or rendering mapped study resources.');
...
console.log('Printable study summary page, privacy boundary, interaction-based mapped resources, learner-name default, read-only controller, JSON/CSV downloads, print CSS, script order, and Reports route contracts passed.');
