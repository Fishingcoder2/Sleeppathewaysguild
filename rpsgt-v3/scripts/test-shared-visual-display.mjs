...
for(const token of ['makeTraceFullscreenButton','ensureTraceFrame','spg-trace-fullscreen-frame','spg-trace-fullscreen-button','View this tracing full screen','data-spg-request-fullscreen'])if(!respJs.includes(token))throw new Error(`Respiratory tracing fullscreen logic missing ${token}`);
...
if(respiratoryToolbarTemplate.includes('data-spg-request-fullscreen'))throw new Error('Respiratory question toolbar must not carry Full screen; Full screen belongs on the waveform.');
...
console.log('Shared visual display passed: Full screen is attached directly to visual traces/viewers, phone landscape split/full-question modes and portrait rotate guidance are present, and Visual Skills/Respiratory remain event-driven without observers or resize loops.');
