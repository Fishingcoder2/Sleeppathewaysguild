...
const task=tasks[0];
const questions=Array.from({length:12},(_,index)=>({id:`q-${index}`,taskCode:task.code,prompt:`Complete question prompt ${index}?`,options:['A','B'],answer:index%2?'A':'B',qa:{manualReviewRecommended:false}}));
...
const first=engine.selectQuestions(questions,task.code,10,'fixed-seed');const second=engine.selectQuestions(questions,task.code,10,'fixed-seed');
assert.deepEqual(first.map(item=>item.id),second.map(item=>item.id));assert.equal(first.length,10);assert.ok(first.every(item=>!['manual','truncated-dots','truncated-ellipsis','placeholder','blank-option'].includes(item.id)));assert.equal(JSON.stringify(questions),sourceQuestions);
...
const eightCorrect=Object.fromEntries(first.map((question,index)=>[question.id,index<8?question.answer:(question.answer==='A'?'B':'A')]));
const passed=engine.gradeCheckpoint({taskCode:task.code,questions:first,answers:eightCorrect,completedAt:'2026-08-02T12:05:00.000Z'});assert.equal(passed.score,80);assert.equal(passed.passed,true);assert.equal(passed.correct,8);assert.equal(passed.total,10);
...
console.log('Guided Trail engine, incomplete-stem exclusion, 10-question task checkpoint, award, legacy-normalization, and immutability contracts passed.');
