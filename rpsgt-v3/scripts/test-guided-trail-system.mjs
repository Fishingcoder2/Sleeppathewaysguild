...
const task=tasks[0];
const questions=Array.from({length:18},(_,index)=>({id:`q-${index}`,taskCode:task.code,prompt:`Complete question prompt ${index}?`,options:['A','B'],answer:index%2?'A':'B',qa:{manualReviewRecommended:false}}));
...
const passed=engine.gradeCheckpoint({taskCode:task.code,questions:first,answers:twelveCorrect,completedAt:'2026-08-02T12:05:00.000Z'});assert.equal(passed.score,80);assert.equal(passed.passed,true);assert.equal(passed.correct,12);assert.equal(passed.total,15);
...
console.log('Guided Trail engine, incomplete-stem exclusion, 15-question task checkpoint, award, legacy-normalization, and immutability contracts passed.');
