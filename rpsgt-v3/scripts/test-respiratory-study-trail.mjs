...
const [trailText,ppsmText,papFamilyText,studyHtml,trailJs,trailCss,homeJs,labsJs,engineJs,explorerJs,completionJs,studyJs]=await Promise.all([
  ...
  readFile(join(root,'core','guided-trail-explorer.js'),'utf8'),
  readFile(join(root,'core','guided-study-completion.js'),'utf8'),
  readFile(join(root,'core','study.js'),'utf8')
]);
...
// The two layers that wrap selectQuestions must preserve the fifth filter argument.
if(!explorerJs.includes('engine.selectQuestions=function(records,taskCode,count,seed,filter)')||!explorerJs.includes("'|concept-pool',filter")) throw new Error('Explorer fresh-question rotation can bypass concept filters.');
if(!completionJs.includes('engine.selectQuestions=function(records,taskCode,count,seed,filter)')||!completionJs.includes('seed,filter)')) throw new Error('Guided Study completion/retake wrapper can bypass concept filters.');
if(!studyJs.includes('conceptId:state.checkpoint.conceptId')||!studyJs.includes('conceptLabel:state.checkpoint.conceptLabel')) throw new Error('Guided Study scoring is not persisting respiratory concept identity.');
...
for(const token of ['data-respiratory-step','Your learning path','respiratory-learning-path','respiratory-path-node','Continue here','Learn this first','Apply it','Check understanding','Finish lesson','Pass checkpoint to unlock next lesson','conceptMastery','respiratory-mastery-stars','Take 15-question concept checkpoint','data-checkpoint-concept','data-checkpoint-concept-label','queueQuestionFilter','this respiratory concept','Score at least 80% to unlock the next lesson','data-respiratory-next','data-respiratory-prev','respiratoryStudyTrail','completedThroughStepId','Related reference materials','Extra practice questions','data-checkpoint-start','Ask Coach Bob from inside a question','sources-disclosures.html?','practice.html?task=','verifiedChapterLocators','data-respiratory-task-entry','Open Respiratory/PAP Study Trail','Best textbook study support','Book: <em>','state.source=ppsm']) if(!trailJs.includes(token)) throw new Error(`Respiratory trail controller is missing ${token}.`);
if(!trailJs.includes("if(!active||state.activeIndex>state.completedThrough&&!conceptMastery(active).passed) return")) throw new Error('Respiratory lesson continuation can bypass concept mastery.');
...
new Function(trailJs);new Function(studyJs);new Function(explorerJs);new Function(completionJs);
