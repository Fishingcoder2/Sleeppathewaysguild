...
const directoryLabel=basename(directory)||'private-exports';
let files=[];
...
catch(error){console.error(JSON.stringify({directory:directoryLabel,error:error.message},null,2));process.exit(2);}
if(!files.length){console.error(JSON.stringify({directory:directoryLabel,error:'No private browser export JSON files were found. Capture an export from migration-export.html first.'},null,2));process.exit(2);}
